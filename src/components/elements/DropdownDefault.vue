...
const defaultOption = { value: 'Select...' };
...
<template>
  ...
  <div class="relative">
    <button type="button" :id="id" :name="id" @click="toggleDropdown" 
      class="group/trigger flex w-full select-none items-center justify-between gap-2 truncate rounded-md border mt-2 px-3 py-2 shadow-sm outline-none transition sm:text-sm border-gray-300 dark:border-gray-800 text-gray-900 dark:text-gray-50 bg-white dark:bg-gray-950 hover:bg-gray-50 hover:dark:bg-gray-950/50 focus:ring-2 focus:ring-blue-200 focus:dark:ring-blue-700/30 focus:border-blue-500 focus:dark:border-blue-700">
      <span class="truncate">
        <span style="pointer-events: none;" :class="{'text-gray-500 dark:text-gray-500': selectedOption.value === 'Select...'}">{{ selectedOption.value }}</span>
      </span>
    ...
    <ul id="dropdown-options" v-show="isOpen"
      ...
      <li v-for="option in options" :key="option.value" :value="option.value" @click="selectOption(option)"
        ...
        <span v-if="option.value === selectedOption.value">
          <svg class="absolute top-2 right-3 w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
            viewBox="0 0 24 24">
